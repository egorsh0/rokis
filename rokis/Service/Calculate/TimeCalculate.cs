...
{
    double K(int timeSpan, double average, double min, double max);
}

public class TimeCalculate : ITimeCalculate
{
    public double K(int timeSpan, double average, double min, double max)
    {
        var r = R(timeSpan, average);
    ...
    internal Func<double, double, double> R { get; } =
        (timeSpan, average) => timeSpan / average;
